# Olá, eu sou o Márcio 👋
### Desenvolvedor Full Stack • Especialista no Ecossistema React & Node.js

<p align="left">
  <a href="https://portfolio-marcio.vercel.app">
    <img src="https://img.shields.io/badge/🌐_Portfólio_&_Hub-Acessar-0070F3?style=for-the-badge" alt="Portfólio" />
  </a>
  <a href="https://linkedin.com">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" />
  </a>
</p>

---

### 🚀 Sobre Mim
- 💼 Foco em desenvolvimento de aplicações completas, escaláveis e com design premium.
- 🛠️ Trabalho principalmente com **Next.js (App Router)**, **React**, **TypeScript**, **Tailwind CSS**, **Prisma ORM** e **PostgreSQL no Supabase**.
- 📖 Escrevendo reflexões e aprendizados diários no meu **Diário Técnico**.

---

### 🛠️ Tecnologias & Ferramentas

<p align="left">
  <img src="https://skillicons.dev/icons?i=nextjs,react,ts,tailwind,prisma,postgres,supabase,nodejs,git,github,vscode" />
</p>

---

### 📊 Estatísticas do GitHub

<p align="center">
  <img src="https://streak-stats.demolab.com?user=MarcioJSMJr&theme=tokyonight&hide_border=true" alt="GitHub Streak Stats" />
</p>

<p align="center">
  <img src="https://github-readme-activity-graph.vercel.app/graph?username=MarcioJSMJr&theme=tokyo-night&hide_border=true" alt="Gráfico de Atividade" />
</p>
